...
namespace detail
{
	...
	/// Marks an enum as being comparable (values being greater or less has meaning)
	template <typename T>
	struct enum_is_comparable : public std::false_type {};
	...
	struct enum_bool_helper
	{
		...
		constexpr operator bool() const { return static_cast<typename std::underlying_type<Enum>::type>(value); }
	};
};

#define MARK_ENUM_AS_FLAGS(T) template<> struct detail::enum_is_flags<T> : public std::true_type {}
#define MARK_ENUM_AS_COMPARABLE(T) template<> struct detail::enum_is_comparable<T> : public std::true_type {}

template <typename Enum>
constexpr typename std::enable_if<detail::enum_is_flags<Enum>::value, Enum>::type
operator |(Enum lhs, Enum rhs) noexcept
{
	using underlying = typename std::make_unsigned<typename std::underlying_type<Enum>::type>::type;
...
template <typename Enum>
constexpr typename std::enable_if<detail::enum_is_comparable<Enum>::value, bool>::type
operator <(Enum lhs, Enum rhs) noexcept
{
