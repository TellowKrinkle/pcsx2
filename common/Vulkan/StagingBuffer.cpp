...
namespace Vulkan
{
	...
	StagingBuffer::StagingBuffer(StagingBuffer&& move)
		...
		, m_map_pointer(move.m_map_pointer)
		, m_map_offset(move.m_map_offset)
	...
	bool StagingBuffer::Map(VkDeviceSize offset, VkDeviceSize size)
	{
		...
		pxAssert(!m_map_pointer);
		pxAssert(m_map_offset + m_map_size <= m_size);
		...
		VkDeviceSize size)
	{
		...
		pxAssert((offset + size) <= m_size || (offset < m_size && size == VK_WHOLE_SIZE));
	...
	{
		pxAssert((offset + size) <= m_size);
		pxAssert(offset >= m_map_offset && size <= (m_map_size + (offset - m_map_offset)));
