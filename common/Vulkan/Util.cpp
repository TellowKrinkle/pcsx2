...
namespace Vulkan
{
	namespace Util
	{
		...
		DebugScope<VkCommandBuffer>::DebugScope(VkCommandBuffer context, const char* format, ...)
		...
		DebugScope<VkQueue>::DebugScope(VkQueue context, const char* format, ...)
			: queue(context)
		{
			if (queue)
			{
				...
				const float depth_phase = depth / static_cast<float>(max_depth);
				BeginDebugScope(queue, str.c_str(),
