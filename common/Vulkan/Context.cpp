...
namespace Vulkan
{
	...
	bool Context::CreateCommandBuffers()
	{
		...
		for (FrameResources& resources : m_frame_resources)
		{
			...
			VkCommandBufferAllocateInfo buffer_info = {VK_STRUCTURE_TYPE_COMMAND_BUFFER_ALLOCATE_INFO, nullptr,
				resources.command_pool, VK_COMMAND_BUFFER_LEVEL_PRIMARY, static_cast<u32>(resources.command_buffers.size())};
			...
			{
				Vulkan::Util::SetObjectName(g_vulkan_context->GetDevice(), resources.command_buffers[i], "Frame %u %sCommand Buffer",
					frame_index, (i == 0) ? "Init" : "");
			}
			...
			VkDescriptorPoolCreateInfo pool_create_info = {VK_STRUCTURE_TYPE_DESCRIPTOR_POOL_CREATE_INFO,
				...
				static_cast<u32>(std::size(pool_sizes)),
				pool_sizes};
	...
	void Context::DestroyCommandBuffers()
	{
		for (FrameResources& resources : m_frame_resources)
		{
			...
			{
				vkFreeCommandBuffers(m_device, resources.command_pool, static_cast<u32>(resources.command_buffers.size()), resources.command_buffers.data());
