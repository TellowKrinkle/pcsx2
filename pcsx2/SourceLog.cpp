...
{
	fmt::format_to(std::back_inserter(ascii), "{:<4}({:8.8x} {:8.8x}): ", ((SysTraceLogDescriptor*)m_Descriptor)->Prefix, cpuRegs.pc, cpuRegs.cycle);
}

void SysTraceLog_IOP::ApplyPrefix(std::string& ascii) const
{
	fmt::format_to(std::back_inserter(ascii), "{:<4}({:8.8x} {:8.8x}): ", ((SysTraceLogDescriptor*)m_Descriptor)->Prefix, psxRegs.pc, psxRegs.cycle);
}
