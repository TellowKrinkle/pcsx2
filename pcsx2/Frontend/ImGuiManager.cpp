...
{
	ImFont* const font = s_fixed_font;
	const float scale = s_global_scale;
...
#define DRAW_LINE(text, color) \
	...
	// if (state == System::State::Running)
	{
		...
		if (GSConfig.OsdShowGSStats)
		{
			std::string gs_stats;
			GSgetStats(gs_stats);
			DRAW_LINE(gs_stats.c_str(), IM_COL32(255, 255, 255, 255));
		}
		...
		if (GSConfig.OsdShowResolution)
		{
			...
			DRAW_LINE(text.c_str(), IM_COL32(255, 255, 255, 255));
		}
...
#if 0
		if (show_status_indicators && (IsFastForwardEnabled() || IsTurboEnabled()))
		{
			DRAW_LINE(rewinding ? ICON_FA_FAST_BACKWARD : ICON_FA_FAST_FORWARD, IM_COL32(255, 255, 255, 255));
		}
#endif
	}
#if 0
	else if (show_status_indicators && state == System::State::Paused)
	{
		DRAW_LINE(ICON_FA_PAUSE, IM_COL32(255, 255, 255, 255));
	}
#endif
