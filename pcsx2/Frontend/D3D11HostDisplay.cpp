...
public:
	D3D11HostDisplayTexture(Microsoft::WRL::ComPtr<ID3D11Texture2D> texture,
		...
		u32 levels, u32 samples, bool dynamic)
	...
	bool dynamic /* = false */)
{
	...
	return std::make_unique<D3D11HostDisplayTexture>(std::move(texture), std::move(srv), width, height, layers, levels,
		samples, dynamic);
}
...
bool D3D11HostDisplay::CreateSwapChain(const DXGI_MODE_DESC* fullscreen_mode)
{
	...
	m_using_flip_model_swap_chain =
		!Host::GetBoolSettingValue("EmuCore/GS", "UseBlitSwapChain", false) || fullscreen_mode;
