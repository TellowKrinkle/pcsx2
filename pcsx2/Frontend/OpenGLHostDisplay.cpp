	...
	bool dynamic /* = false */)
{
	...
	if (samples > 1)
	{
		...
		if (GLAD_GL_ARB_texture_storage || GLAD_GL_ES_VERSION_3_1)
			glTexStorage2DMultisample(target, samples, GL_RGBA8, width, height, GL_FALSE);
		else
			glTexImage2DMultisample(target, samples, GL_RGBA8, width, height, GL_FALSE);
	}
	else
	{
...
{
	if (frame_skip || ShouldSkipDisplayingFrame() || m_window_info.type == WindowInfo::Type::Surfaceless)
	{
