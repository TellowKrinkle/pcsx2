...
void VMManager::ApplyGameFixes()
{
	...
	for (SpeedhackId id = SpeedhackId_FIRST; id < pxEnumEnd; id++)
	{
		...
		if (game->speedHacks.count(key) == 1)
		{
			...
			PatchesCon->WriteLn("(GameDB) Setting Speedhack '%s' to [mode=%d]", key.c_str(), mode);
			s_active_game_fixes++;
		}
	}
	...
	for (GamefixId id = GamefixId_FIRST; id < pxEnumEnd; id++)
	{
		...
		if (std::find(game->gameFixes.begin(), game->gameFixes.end(), key) != game->gameFixes.end())
		{
			...
			PatchesCon->WriteLn("(GameDB) Enabled Gamefix: %s", key.c_str());
			s_active_game_fixes++;
...
void VMManager::UpdateRunningGame(bool force)
{
...

	{
		...
		if (const GameDatabaseSchema::GameEntry* game = GameDatabase::FindGame(s_game_serial))
		{
			...
			sioSetGameSerial(StringUtil::UTF8StringToWxString(game->MemcardFiltersAsString()));
		}
		else
		{
			...
			sioSetGameSerial(StringUtil::UTF8StringToWxString(s_game_serial));
		}
	}
...
void VMManager::SetPaused(bool paused)
{
	...
	const bool was_running = (s_state.load() == VMState::Running);
