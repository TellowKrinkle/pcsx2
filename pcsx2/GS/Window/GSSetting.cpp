...
const char* dialog_message(int ID, bool* updateText)
{
	...
	switch (ID)
	{
		...
		case IDC_FULLDEPTH:
			return cvtString("Higher values give more range to depth but reduce precision.  Higher values may cause z-fighting.");
		case IDC_MULTITHREADED_GL:
			return cvtString("Offloads GL processing from the MTGS thread to a separate thread.  Improves performance in MTGS thread limited games.  May cause crashes in some drivers.");
