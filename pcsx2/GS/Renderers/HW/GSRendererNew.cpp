...
void GSRendererNew::SetupIA(const float& sx, const float& sy)
{
	...
	switch (m_vt.m_primclass)
	{
		case GS_POINT_CLASS:
			if (unscale_pt_ln)
			{
				m_conf.gs = GSHWDrawConfig::GSSelector::Point;
				m_conf.cb_vs.PointSize = GSVector2(16.0f * sx, 16.0f * sy);
			}
		...
		case GS_LINE_CLASS:
			if (unscale_pt_ln)
			{
				m_conf.gs = GSHWDrawConfig::GSSelector::Line;
				m_conf.cb_vs.PointSize = GSVector2(16.0f * sx, 16.0f * sy);
			}
		...
		case GS_SPRITE_CLASS:
			...
			{
				m_conf.gs = GSHWDrawConfig::GSSelector::Sprite;
...
void GSRendererNew::EmulateZbuffer()
{
	if (m_context->TEST.ZTE)
	{
		...
		m_conf.depth.zwe = !m_context->ZBUF.ZMSK;
	}
	else
	{
	...
	m_conf.cb_vs.MaxDepth = GSVector2i(0xFFFFFFFF);
	...
	if (clamp_z)
	{
		...
		{
			m_conf.cb_vs.MaxDepth = GSVector2i(max_z);
		}
...
void GSRendererNew::EmulateTextureShuffleAndFbmask()
{
	...
	if (m_texture_shuffle)
	{
		...
		// of shader.
		m_conf.ps.write_rg = !write_ba && m_context->TEST.DATE;
		...
		if (m_conf.ps.fbmask && m_sw_blending)
		{
	...
	else
	{
		...
		m_conf.ps.fbmask = m_sw_blending && (~ff_fbmask & ~zero_fbmask & 0xF);
...
void GSRendererNew::EmulateBlending(bool& DATE_GL42, bool& DATE_GL45)
{
	...
	if (sw_blending)
	{
		...
		if (accumulation_blend)
		{
			...
			m_conf.blend = {blend_index, 0, false, true};
			if (ALPHA.A == 2)
			{
	...
	else
	{
		...
		if (m_conf.ps.dfmt == 1 && ALPHA.C == 1)
		{
			...
			m_conf.blend = {hacked_blend_index, 128, true, false};
		}
		else
		{
			m_conf.blend = {blend_index, ALPHA.FIX, ALPHA.C == 2, false};
		}
	}
}
...
void GSRendererNew::EmulateTextureSampler(const GSTextureCache::Source* tex)
{
	...
	if (m_conf.ps.shuffle)
	{
		...
		if (psm.depth)
		{
			...
			m_conf.vs.int_fst = !PRIM->FST; // select float/int coordinate
		}
	...
	m_conf.ps.point_sampler = GLLoader::vendor_id_amd && (!bilinear || shader_emulated_sampler);
