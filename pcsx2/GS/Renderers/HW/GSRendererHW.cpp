...
void GSRendererHW::HandleProvokingVertexFirst()
{
	...
	{
		if (vtx_buff.buff[idx_buff.buff[i]].RGBAQ.U32[0] != vtx_buff.buff[idx_buff.buff[i + n - 1]].RGBAQ.U32[0])
		{
			first_eq_last = false;
			break;
		}
	...
	for (u32 i = 0; i < idx_buff.tail; i += n)
	{
		vtx_buff.buff[i].RGBAQ.U32[0] = vtx_buff.buff[i + n - 1].RGBAQ.U32[0];
		vtx_buff.buff[i + n - 1].RGBAQ.U32[0] = 0xff; // Make last vertex red for debugging if used improperly
	}
}
