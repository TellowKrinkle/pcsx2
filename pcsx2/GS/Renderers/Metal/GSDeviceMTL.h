...
struct PSSelectorMTL
{
	GSHWDrawConfig::PSSelector base;
	bool tex_is_r8;
	PSSelectorMTL(GSHWDrawConfig::PSSelector base, bool tex_is_r8)
	{
		memset(this, 0, sizeof(*this));
		this->base = base;
		this->tex_is_r8 = tex_is_r8;
	}
	PSSelectorMTL(const PSSelectorMTL& other)
	{
		memcpy(this, &other, sizeof(other));
	}
	bool operator==(const PSSelectorMTL& other) const
	{
...
struct PipelineSelectorExtrasMTL
{
	...
	PipelineSelectorExtrasMTL(GSHWDrawConfig::BlendState blend, GSTexture* rt, GSHWDrawConfig::OMColorMaskSelector cms, bool has_depth, bool has_stencil)
		: key(0)
	{
...
template <>
struct std::hash<PSSelectorMTL>
{
	size_t operator()(const PSSelectorMTL& sel) const
	{
		const char* ptr = reinterpret_cast<const char*>(&sel);
		return std::hash<std::string_view>()(std::string_view(ptr, sizeof(PSSelectorMTL)));
	}
};
...
{
public:
	using DepthStencilSelector = GSHWDrawConfig::OMDepthStencilSelector;
	using SamplerSelector = GSHWDrawConfig::PSSamplerSelector;
	...
	std::unordered_map<PSSelectorMTL, id<MTLFunction>> m_hw_ps;
