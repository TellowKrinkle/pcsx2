...
constant bool PS_TEX_IS_R8          [[function_constant(GSMTLConstantIndex_PS_TEXTURE_IS_R8)]];

//constant bool PS_PRIM_CHECKING_WRITE = PS_DATE == 1 || PS_DATE == 2;
//constant bool PS_PRIM_CHECKING_READ = PS_DATE == 3;
constant bool PS_TEX_IS_DEPTH = PS_DEPTH_FMT == 1 || PS_DEPTH_FMT == 2;
constant bool PS_TEX_IS_COLOR = !PS_TEX_IS_DEPTH;
constant bool PS_HAS_PALETTE = PS_PAL_FMT != 0 || (PS_CHANNEL >= 1 && PS_CHANNEL <= 5);
constant bool PS_HAS_RAW_DEPTH = PS_URBAN_CHAOS_HLE || PS_TALES_OF_ABYSS_HLE || (PS_TEX_IS_DEPTH && PS_CHANNEL);
constant bool PS_HAS_RAW_COLOR = PS_CHANNEL && !PS_HAS_RAW_DEPTH;
...
struct PSMain
{
	...
	texture2d<float> tex_raw;
	depth2d<float> tex_raw_depth;
	...
	float4 clamp_wrap_uv(float4 uv)
	{
		...
		if (PS_WMS == PS_WMT)
		{
			...
			else if (PS_WMS == 3)
			{
				...
				uv_out = float4((ushort4(uv * tex_size) & cb.uv_msk_fix.xyxy) | cb.uv_msk_fix.zwzw) / tex_size;
			}
		}
		else
		{
			...
			else if (PS_WMS == 3)
			{
				...
				uv_out.xz = float2((ushort2(uv.xz * tex_size.xx) & cb.uv_msk_fix.xx) | cb.uv_msk_fix.zz) / tex_size.xx;
			}
			...
			else if (PS_WMT == 3)
			{
				...
				uv_out.yw = float2((ushort2(uv.yw * tex_size.yy) & cb.uv_msk_fix.yy) | cb.uv_msk_fix.ww) / tex_size.yy;
			}
		}
	...
	float4 sample_4_index(float4 uv)
	{
		...
		uchar4 i = uchar4(c * 255.1f); // Denormalize value
	...
	{
		return tex_raw_depth.read(ushort2(in.p.xy)) * 0x1p32f;
	}

	float4 fetch_raw_color()
	{
		return tex_raw.read(ushort2(in.p.xy));
	}
	...
	float4 fetch_gXbY()
	{
		...
		{
			uchar4 rt = uchar4(fetch_raw_color() * 255.1f);
	...
	{
		if (PS_FBMASK)
			C = float4((uchar4(C) & ~cb.fbmask) | (current_color_int & cb.fbmask));
	}
	...
	void ps_dither(thread float4& C)
	{
		...
		// dither_matrix is a 4x4 matrix of 3-bit values with alignment of 4 bits
		// Index with [fpos.y & 3][fpos.x & 3]
		int dither = fpos.y & 2 ? cb.dither_matrix.y : cb.dither_matrix.x;
		int index = ((fpos.y & 1) << 2) | (fpos.x & 3);
		C.rgb += (dither << (29 - index * 4)) >> 29;
	}
	...
	void ps_blend(thread float4& Color, float As)
	{
		...
		if (PS_PABE)
			Color.rgb = (As >= 128.f) ? Color.rgb : Cs;
	...
	MainPSOut ps_main()
	{
		...
		if (PS_SHUFFLE)
		{
			...
			uchar2 denorm_TA = uchar2(cb.ta * 255.1f);
		...
		if (!SW_BLEND && PS_DITHER && PS_DFMT == FMT_16 && !PS_COLCLIP)
		{
			...
			C.rgb = float3(uchar3(C.rgb) & 0xF8);
		}
...
fragment MainPSOut ps_main(
	...
	texture2d<float> tex_raw   [[texture(GSMTLTextureIndexRawTex),       function_constant(PS_HAS_RAW_COLOR)]],
	depth2d<float>   depth_raw [[texture(GSMTLTextureIndexRawTex),       function_constant(PS_HAS_RAW_DEPTH)]],
	texture2d<float> rt        [[texture(GSMTLTextureIndexRenderTarget), function_constant(NEEDS_RT)]])
{
	...
	if (PS_HAS_RAW_COLOR)
		main.tex_raw = tex_raw;
	if (PS_HAS_RAW_DEPTH)
		main.tex_raw_depth = depth_raw;

	if (NEEDS_RT)
	{
		main.current_color = rt.read(ushort2(in.p.xy));
		main.current_color_int = uchar4(main.current_color * 255.1f);
	}
	else
	{
