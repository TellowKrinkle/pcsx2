...
bool GSDeviceOGL::Create(const WindowInfo& wi)
{
	...
	{
		auto shader = Host::ReadResourceFileToString("common_header.glsl");
		if (!shader.has_value())
			return false;
	...
	GLuint ps = 0;
	{
		...
		const auto shader = Host::ReadResourceFileToString("convert.glsl");
		if (!shader.has_value())
			return false;
		...
		{
			ps = m_shader->CompileShader("convert.glsl", format("ps_main%d", i), GL_FRAGMENT_SHADER, m_shader_common_header, shader->c_str());
	...
	// ****************************************************************
	{
		...
		const auto shader = Host::ReadResourceFileToString("merge.glsl");
		if (!shader.has_value())
			return false;
	...
	// ****************************************************************
	{
		...
		const auto shader = Host::ReadResourceFileToString("interlace.glsl");
		if (!shader.has_value())
			return false;
	...
	// ****************************************************************
	{
		...
		const auto shader = Host::ReadResourceFileToString("shadeboost.glsl");
		if (!shader.has_value())
			return false;
...
bool GSDeviceOGL::CreateTextureFX()
{
	...
	auto vertex_shader = Host::ReadResourceFileToString("tfx_vgs.glsl");
	auto fragment_shader = Host::ReadResourceFileToString("tfx_fs.glsl");
...
void GSDeviceOGL::DoFXAA(GSTexture* sTex, GSTexture* dTex)
{
	...
	if (!m_fxaa.ps)
	{
		...
		std::optional<std::string> shader = Host::ReadResourceFileToString("fxaa.fx");
		if (!shader.has_value())
			return;
