	...
	, m_shader(NULL)
{
	...
	m_features.texture_barrier = GLExtension::Has("GL_ARB_texture_barrier");
}
...
bool GSDeviceOGL::Create(const WindowInfo& wi)
{
	...
	GLuint ps = 0;
	{
		...
		vs = m_shader->CompileShader("convert.glsl", "vs_main", GL_VERTEX_SHADER, shader.data());

		m_convert.vs = vs;
		for (size_t i = 0; i < std::size(m_convert.ps); i++)
		{
			ps = m_shader->CompileShader("convert.glsl", format("ps_main%d", i), GL_FRAGMENT_SHADER, shader.data());
	...
	// ****************************************************************
	{
		...
		{
			ps = m_shader->CompileShader("merge.glsl", format("ps_main%d", i), GL_FRAGMENT_SHADER, shader.data());
	...
	// ****************************************************************
	{
		...
		{
			ps = m_shader->CompileShader("interlace.glsl", format("ps_main%d", i), GL_FRAGMENT_SHADER, shader.data());
	...
	// ****************************************************************
	{
		...
		ps = m_shader->CompileShader("shadeboost.glsl", "ps_main", GL_FRAGMENT_SHADER, shader.data(), shade_macro);
...
void GSDeviceOGL::StretchRect(GSTexture* sTex, const GSVector4& sRect, GSTexture* dTex, const GSVector4& dRect, GLuint ps, int bs, OMColorMaskSelector cms, bool linear)
{
	...
	const bool draw_in_depth = (ps == m_convert.ps[(int)ShaderConvert::RGBA8_TO_FLOAT32] || ps == m_convert.ps[(int)ShaderConvert::RGBA8_TO_FLOAT24] ||
		ps == m_convert.ps[(int)ShaderConvert::RGBA8_TO_FLOAT16] || ps == m_convert.ps[(int)ShaderConvert::RGB5A1_TO_FLOAT16]);
...
void GSDeviceOGL::RenderOsd(GSTexture* dt)
{
	...
	m_shader->BindProgram(m_convert.ps[(int)ShaderConvert::OSD]);
...
void GSDeviceOGL::DoFXAA(GSTexture* sTex, GSTexture* dTex)
{
	...
	if (!m_fxaa.ps)
	{
		...
		GLuint ps = m_shader->CompileShader("fxaa.fx", "ps_main", GL_FRAGMENT_SHADER, shader.data(), fxaa_macro);
...
void GSDeviceOGL::DoExternalFX(GSTexture* sTex, GSTexture* dTex)
{
	...
	if (!m_shaderfx.ps)
	{
		...
		GLuint ps = m_shader->CompileShader("Extra", "ps_main", GL_FRAGMENT_SHADER, shader.str().c_str(), config.str());
...
void GSDeviceOGL::SetupDATE(GSTexture* rt, GSTexture* ds, const GSVertexPT1* vertices, bool datm)
{
	...
	m_shader->BindProgram(m_convert.ps[(int)(datm ? ShaderConvert::DATM_1 : ShaderConvert::DATM_0)]);
...
GSDeviceOGL::VSConstantBuffer GSDeviceOGL::ConvertCB(const GSHWDrawConfig::VSConstantBuffer& cb)
{
	VSConstantBuffer out;
	...
	out.PointSize = cb.PointSize;
	out.MaxDepth = cb.MaxDepth;
	return out;
}

GSDeviceOGL::PSConstantBuffer GSDeviceOGL::ConvertCB(const GSHWDrawConfig::PSConstantBuffer& cb, int atst)
{
	PSConstantBuffer out;
	out.FogColor_AREF = GSVector4(GSVector4i::load(cb.fog_color_aref).u8to32());
	if (atst == 1 || atst == 2) // Greater / Less alpha
		out.FogColor_AREF.w -= 0.1;
	out.WH = cb.texture_size;
	out.TA_Af = GSVector4(GSVector4i::load(cb.ta_af).u8to32()) / GSVector4(255.f, 255.f, 1.f, 128.f);
	...
	out.MaxDepth = GSVector4(0.f, 0.f, 0.f, cb.max_depth * ldexpf(1, -32));

	GSVector4i dither = GSVector4i::loadl(&cb.dither_matrix).u8to16();
	GSVector4i ditherLow = dither.sll16(13).sra16(13);
	GSVector4i ditherHi  = dither.sll16( 9).sra16( 5);
...
void GSDeviceOGL::RenderHW(GSHWDrawConfig& config)
{
	...
	OMSetBlendState(config.blend.index, config.blend.factor, config.blend.is_constant, config.blend.is_accumulation);
	...
	switch (config.gs)
	{
		case GSHWDrawConfig::GSSelector::Point:  gssel.point  = 1; break;
		case GSHWDrawConfig::GSSelector::Line:   gssel.line   = 1; break;
		case GSHWDrawConfig::GSSelector::Sprite: gssel.sprite = 1; break;
		case GSHWDrawConfig::GSSelector::None:                     break;
	}
	PSSelector pssel;
	pssel.key = config.ps.key;
	SetupPipeline(VSSelector(config.vs.key), gssel, pssel);
	...
	if (config.ps.hdr)
	{
		...
		hdr_rt = CreateTexture(size.x, size.y, GSTexture::Format::FloatColor);
		CopyRectConv(config.rt, hdr_rt, config.scissor, false);
