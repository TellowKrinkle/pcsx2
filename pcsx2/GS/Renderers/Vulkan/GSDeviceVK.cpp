...
bool GSDeviceVK::CheckFeatures()
{
	...
	for (u32 fmt = static_cast<u32>(GSTexture::Format::Color); fmt < static_cast<u32>(GSTexture::Format::Last); fmt++)
	{
	...
	for (u32 ds = 0; ds < 2; ds++)
	{
		for (u32 clear = 0; clear < 2; clear++)
		{
			m_date_image_setup_render_passes[ds][clear] =
				g_vulkan_context->GetRenderPass(GSTextureVK::LookupNativeFormat(GSTexture::Format::PrimID),
