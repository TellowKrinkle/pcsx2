...
#define _rip_local_(ptrtype, field) ((is32 || m_rip) ? ptrtype[rip + (char*)&m_local.field] : ptrtype[_m_local + OFFSETOF(GSScanlineLocalData, field)])
	...
	if (m_sel.prim != GS_SPRITE_CLASS)
	{
		if (m_sel.fwrite && m_sel.fge || m_sel.zb)
		{
			...
			if (m_sel.zb && m_sel.zequal)
			{
				...
				{
					movsd(zx, ptr[a3 + offsetof(GSVertexSW, p.z)]);
					addsd(zx, ptr[&GSVector4::m_xc1e00000000fffff]);
					cvtsd2si(rax, zx);
					add(eax, 0x80000000);
					movd(zx, eax);
				}
				...
				if (hasAVX2)
					...
					pshufd(z, z, _MM_SHUFFLE(0, 0, 0, 0));

				ONLY32(movaps(_rip_local(temp.z0), z));
			}
			else if (m_sel.zb)
			{
				const XYm& z0 = is64 ? xym7 : xym5;
				// z = vp.zzzz() + m_local.d[skip].z;
				broadcastsd(xym1, ptr[a3 + offsetof(GSVertexSW, p.z)]); // v.p.z
				cvtps2pd(z0, ptr[a1 + offsetof(GSScanlineLocalData::skip, z.I8[0])]);
				addpd(z0, xym1);
				movaps(_rip_local(temp.z0), z0);
				cvtps2pd(z, ptr[a1 + offsetof(GSScanlineLocalData::skip, z.I8[vecsize/2])]);
				addpd(z, xym1);
				ONLY32(movaps(_rip_local(temp.z1), z));
			}
		}
	}
	else
	...
	if (m_sel.fb)
	{
		...
		if (m_sel.tfx != TFX_NONE)
		{
			...
			else
			{
				...
				if (is32)
				{
					movaps(_rip_local(temp.s), s);
					movaps(_rip_local(temp.t), t);
					movaps(_rip_local(temp.q), q);
				}
			}
		}
		...
		if (!(m_sel.tfx == TFX_DECAL && m_sel.tcc))
		{
			...
			if (m_sel.iip)
			{
				...
				ONLY32(movdqa(_rip_local(temp.rb), f_rb));
				ONLY32(movdqa(_rip_local(temp.ga), f_ga));
			}
...
void GSDrawScanlineCodeGenerator2::Step()
{
	...
	if (m_sel.prim != GS_SPRITE_CLASS)
	{
		...
		if (m_sel.zb && !m_sel.zequal)
		{
			...
			ONLY32(movaps(z, _rip_local(temp.z1)));
			addpd(z, xym7);
			addpd(xym7, _rip_local(temp.z0));
			movaps(_rip_local(temp.z0), xym7);
			ONLY32(movaps(_rip_local(temp.z1), z));
		}
		else if (m_sel.zb && is32)
		{
			movdqa(z, _rip_local(temp.z0));
		}
...
void GSDrawScanlineCodeGenerator2::TestZ(const XYm& temp1, const XYm& temp2)
{
	...
	if (m_sel.prim != GS_SPRITE_CLASS)
	{
		...
		else if (m_sel.zoverflow && is32)
		{
			broadcastsd(temp1, ptr[&GSVector4::m_xc1e00000000fffff]);
			addpd(xym0, temp1);
			addpd(temp1, _rip_local(temp.z0));
			cvtpd2dq(Xmm(temp2.getIdx()), xym0);
			cvtpd2dq(xmm0, temp1);
#if USING_YMM
			vinserti128(xym0, xym0, Xmm(temp2.getIdx()), 1);
#else
			punpcklqdq(xym0, temp2);
#endif
			pcmpeqd(temp1, temp1);
			pslld(temp1, 31);
			paddd(xym0, temp1);
		...
		{
			cvttpd2dq(Xmm(temp1.getIdx()), xym0);
#if USING_YMM
			cvttpd2dq(xmm0, yword[&m_local.temp.z0]);
			vinserti128(xym0, xym0, Xmm(temp1.getIdx()), 1);
#else
			cvttpd2dq(xmm0, xword[&m_local.temp.z0]);
			punpcklqdq(xym0, temp1);
#endif
		}
