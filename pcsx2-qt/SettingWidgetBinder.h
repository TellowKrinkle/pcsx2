...
template<typename WidgetType>
static void BindWidgetToIntSetting(WidgetType* widget, std::string section, std::string key, int default_value)
{
  using Accessor = SettingAccessor<WidgetType>;

  s32 value = QtHost::GetBaseIntSettingValue(section.c_str(), key.c_str(), static_cast<s32>(default_value));

  Accessor::setIntValue(widget, static_cast<int>(value));

  Accessor::connectValueChanged(widget, [widget, section, key]() {
    const int new_value = Accessor::getIntValue(widget);
    QtHost::SetBaseIntSettingValue(section.c_str(), key.c_str(), new_value);
    g_emu_thread->applySettings();
  });
}
