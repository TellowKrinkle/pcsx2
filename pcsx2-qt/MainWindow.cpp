...
void MainWindow::connectSignals()
{
  ...
  connect(m_ui.actionDebugSettings, &QAction::triggered, [this]() { doDebugSettings(); });
...
DisplayWidget* MainWindow::createDisplay(bool fullscreen, bool render_to_main)
{
  ...
  if (!host_display->CreateRenderDevice(wi.value(), QtHost::GetBaseStringSettingValue("EmuCore/GS", "Adapter", ""),
                                        QtHost::GetBaseBoolSettingValue("EmuCore/GS", "ThreadedPresentation", false),
                                        QtHost::GetBaseBoolSettingValue("EmuCore/GS", "UseDebugDevice", false)))
  {
...
DebugSettingsDialog* MainWindow::getDebugSettingsDialog()
{
  if (!m_debug_settings_dialog)
    m_debug_settings_dialog = new DebugSettingsDialog(this);

  return m_debug_settings_dialog;
}
...
void MainWindow::doDebugSettings(DebugSettingsDialog::Category category)
{
  DebugSettingsDialog* dlg = getDebugSettingsDialog();
  if (!dlg->isVisible())
  {
    dlg->setModal(false);
    dlg->show();
  ...
  if (category != DebugSettingsDialog::Category::Count)
    dlg->setCategory(category);
}
